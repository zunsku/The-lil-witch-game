...
namespace PixelCrushers.DialogueSystem.ArcweaveSupport
{
    ...
    public class ArcweaveImporter
    {
        ...
        {
            var id = template.GetNextDialogueEntryID(conversation);
            var child = template.CreateDialogueEntry(id, conversation.id, $"if {condition}");
            conversation.dialogueEntries.Add(child);
        ...
        {
            var id = template.GetNextDialogueEntryID(conversation);
            var child = template.CreateDialogueEntry(id, conversation.id, $"elseif {condition}");
            conversation.dialogueEntries.Add(child);
            entry.outgoingLinks.Add(new Link(conversation.id, entry.id, conversation.id, child.id));
            child.conditionsString = ConvertArcscriptToLua(condition);
            ConvertTokensToNodes(conversation, child, tokens);
        ...
        {
            var id = template.GetNextDialogueEntryID(conversation);
            var child = template.CreateDialogueEntry(id, conversation.id, $"else");
            conversation.dialogueEntries.Add(child);
            entry.outgoingLinks.Add(new Link(conversation.id, entry.id, conversation.id, child.id));
            ConvertTokensToNodes(conversation, child, tokens);
        }
